...
export async function carregarSocios(supabase, filtros) {
  try {
    ...
    let query = supabase
      .from('socios')
      .select('id,nome_completo,cpf,email,whatsapp,empresa,cargo,segunda_empresa,status,data_filiacao,adimplente')
...
export function exportarParaCSV(socios, nomeArquivo = 'socios.csv') {
  ...
  // Headers do CSV
  const headers = ['Nome', 'CPF', 'Email', 'Telefone/WhatsApp', 'Local de Trabalho', 'Cargo', 'Segunda Empresa (saúde)', 'Status', 'Adimplente', 'Data de Filiação']
