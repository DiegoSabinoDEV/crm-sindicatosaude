...
const contrachequeInput = document.getElementById('contracheque')
...
for (const radio of document.querySelectorAll('input[name="forma_pagamento"]')) {
    ...
    try {
      await fetch('https://n8n.liftcode.com.br/webhook/pesquisa-sindicato', {
        ...
        body: JSON.stringify({
          ...
          link_crm: 'https://crm.portalsinteenp.org/dashboard.html'
        })
      })
    } catch {}
...
function coletarDadosFormulario() {
  ...
  return {
    ...
    sexo: valorCampo('sexo') || null,
    estado_civil: valorCampo('estado_civil') || null,
    ...
    matricula: valorCampo('ctps_numero'),
    data_admissao: valorCampo('data_admissao') || null,
    forma_pagamento: obterFormaPagamento(),
    contracheque,
...
function validarCampos(dados) {
  ...
  if (dados.forma_pagamento === 'direto' && !dados.contracheque) {
    throw new Error('Envie o contracheque para continuar.')
  }

  if (dados.contracheque && !tiposContrachequeValidos().includes(dados.contracheque.type)) {
    throw new Error('Envie um contracheque em PDF, JPG ou PNG.')
  }

  if (dados.contracheque && dados.contracheque.size > 2 * 1024 * 1024) {
    throw new Error('O contracheque deve ter no máximo 2MB. Compacte o arquivo e tente novamente.')
  }
...
function atualizarFormaPagamento() {
  ...
  paymentNote?.classList.toggle('is-visible', forma === 'direto')

  if (contrachequeInput) {
    contrachequeInput.required = forma === 'direto'
  }
}
...
function mapearMensagemErro(error) {
  ...
  if (mensagemOriginal === 'Aceite os termos para continuar.') {
    return mensagemOriginal
  }
...
function exibirErro(mensagem) {
  ...
  formMessage.className = 'message error is-visible'
...
function obterFormaPagamento() {
  return document.querySelector('input[name="forma_pagamento"]:checked')?.value || ''
}
