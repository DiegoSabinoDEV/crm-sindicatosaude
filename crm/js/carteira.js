...
async function gerarCanvasCarteira() {
  ...
  // ── Logo ──
  const logoImg = await carregarImagem('/logo/logoSinteenp_transparente.png')
  if (logoImg) {
    const maxH  = 85
  ...
  gradBottom.addColorStop(0, 'rgba(204,0,0,0)')
  gradBottom.addColorStop(1, 'rgba(204,0,0,0.16)')
  ...
  if (currentCarteira?.id) {
    const verifyUrl = `https://portalsinteenp.org/verificar.html?id=${currentCarteira.id}`
...
document.getElementById('btn-download').addEventListener('click', () => {
  ...
  link.download = `carteira-sinteenp-${(currentSocio?.cpf || '').replace(/\D/g, '')}.png`
...
if (navigator.share && navigator.canShare) {
  ...
  btnShare.addEventListener('click', () => {
    document.getElementById('card-canvas').toBlob(async blob => {
      const file = new File([blob], 'carteira-sinteenp.png', { type: 'image/png' })
